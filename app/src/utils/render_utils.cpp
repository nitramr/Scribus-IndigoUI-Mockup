...
void RenderUtils::renderPattern(QPainter *painter, QRectF rect)
{
...
void RenderUtils::renderCircularHandle(QPainter *painter, QPointF center, qreal width, QBrush background)
{
...
void RenderUtils::renderPointerHandle(QPainter *painter, QPointF pointer, qreal width, QColor background)
{
...
QPixmap RenderUtils::renderSplitColor(QSize size, QColor color, Qt::Orientations orientation, qreal mid)
{
    mid = qBound(0., mid, 1.);
...
void RenderUtils::paintIcon(const QPixmap &toPaint, QPixmap &target, int x, int y)
{
    QPainter p;
    p.begin(&target);
