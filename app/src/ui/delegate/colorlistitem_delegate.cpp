...
void ColorFancyItemDelegate::redraw(const QVariant& data) const
{
    ...
    if (data.canConvert<ColorPixmapValue>())
    {
        ...
        RenderUtils::paintIcon(smallPix, *pPixmap, 0, 0);

        if (item.m_color.isOutOfGamut())
            RenderUtils::paintIcon(alertIcon.scaled(QSize(16,16), Qt::KeepAspectRatio, Qt::SmoothTransformation), *pPixmap, 18, 0);


        switch(item.m_color.getColorModel()){
        case ColorModel::CMYK:
            RenderUtils::paintIcon(cmykIcon.scaled(QSize(16,16), Qt::KeepAspectRatio, Qt::SmoothTransformation), *pPixmap, 36, 0);
            break;
        case ColorModel::HSV:
        case ColorModel::RGB:
            RenderUtils::paintIcon(rgbIcon.scaled(QSize(16,16), Qt::KeepAspectRatio, Qt::SmoothTransformation), *pPixmap, 36, 0);
            break;
        case ColorModel::Lab:
            RenderUtils::paintIcon(labIcon.scaled(QSize(16,16), Qt::KeepAspectRatio, Qt::SmoothTransformation), *pPixmap, 36, 0);
            break;
        ...
        if (item.m_color.isSpotColor()){
            RenderUtils::paintIcon(spotIcon.scaled(QSize(16,16), Qt::KeepAspectRatio, Qt::SmoothTransformation), *pPixmap, 54, 0);
        }


        if (item.m_color.isRegistrationColor()){
            RenderUtils::paintIcon(regIcon.scaled(QSize(16,16), Qt::KeepAspectRatio, Qt::SmoothTransformation), *pPixmap, 54, 0);
        }
    }
}
